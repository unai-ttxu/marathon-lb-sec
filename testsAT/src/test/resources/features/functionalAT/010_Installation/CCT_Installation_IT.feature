...
Feature: [QATM-1870] Service_Installation
  ...
  Scenario: [QATM-1870][02] Marathon-LB Installation - Status
    ...
    And I securely send requests to '${CLUSTER_ID}.${CLUSTER_DOMAIN:-labs.stratio.com}:443'
    #Check status in API
    When in less than '500' seconds, checking each '20' seconds, I send a 'GET' request to '/service/deploy-api/deploy/status/service?service=/${SERVICE:-marathon-lb-sec}' so that the response contains '"healthy":1'
    #Check status in DCOS
    When I open a ssh connection to '${DCOS_CLI_HOST}' with user 'root' and password 'stratio'
    Then in less than '500' seconds, checking each '20' seconds, the command output 'dcos task | grep ${SERVICE:-marathon-lb-sec} | grep R | wc -l' contains '1'
    When I run 'dcos task |  awk '{print $5}' | grep ${SERVICE:-marathon-lb-sec}' in the ssh connection and save the value in environment variable 'marathonlb-TaskId'
