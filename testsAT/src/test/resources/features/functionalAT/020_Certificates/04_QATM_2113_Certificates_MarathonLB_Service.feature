...
Feature: [QATM-2113] Certificates MarathonLB service
  ...
  Scenario:[02] Check Vault path by default
    ...
    And I run 'curl -X GET -fskL --tlsv1.2 -H "X-Vault-Token:!{VAULT_TOKEN}" "https://!{EOS_VAULT_HOST}:${EOS_VAULT_PORT}/v1/${VAULT_USERLAND_CERTIFICATE_BASE_PATH:-userland/certificates/}marathon-lb" | jq '.data."marathon-lb.marathon.mesos_key"'' locally with exit status '0'
    And I run 'curl -X GET -fskL --tlsv1.2 -H "X-Vault-Token:!{VAULT_TOKEN}" "https://!{EOS_VAULT_HOST}:${EOS_VAULT_PORT}/v1/${VAULT_USERLAND_CERTIFICATE_BASE_PATH:-userland/certificates/}marathon-lb" | jq '.data."marathon-lb.marathon.mesos_crt"'' locally with exit status '0'
