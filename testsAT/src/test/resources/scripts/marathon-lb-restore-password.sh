...
curl -k -H "X-Vault-Token:${VAULT_TOKEN}" -H "Content-Type:application/json" -X POST -d @/stratio_volume/marathon-mesos-passw-backup.json https://vault.service.${INTERNAL_DOMAIN}:8200/v1/dcs/passwords/marathon/mesos
curl -k -H "X-Vault-Token:${VAULT_TOKEN}" -H "Content-Type:application/json" -X POST -d @/stratio_volume/marathon-rest-passw-backup.json https://vault.service.${INTERNAL_DOMAIN}:8200/v1/dcs/passwords/marathon/rest
